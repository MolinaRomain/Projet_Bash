...
#   En cours : ajout md5 pour dossiers
...
TriFD(){
    ...
    temp2=`mktemp TEMP_XXX`
    fichierTri=`mktemp TEMP_XXX`
    cat $fichier
    echo ""
    cat $fichier | tail -n +2 > $temp
    cat $temp > $fichier
#    if [ -z "$dir2" ]
#    then
#        cmdDir="cat $fichier"
...
#      cat $fichier | $cmdDir | sed -e s/$dir1//g | tr / '\n' > $temp #sinon on remplace seulement les dir1 et on va à la ligne à chaque /
        #printf $dir1'\n'$dir2 >> $temp
#       awk NF $temp > $fichier #on remove les lignes vides
        #cat $fichier
        > $temp #erase le fichier temp
        #cat $fichier
        #echo "FICHIER"
        #sortie des md5 pour chaque fichier dans un dossier
        while read ligne
        do
            #echo "Le dossier $ligne contient :" >> $temp
            #echo $ligne
            namedir=$ligne".txt"
            for pointeur in `find $ligne -type f`
            do
                md5sum $pointeur | cut -d' ' -f1 >> $namedir

            done
            for pointeur in `find $ligne -type d`
            do
                if [ "$pointeur" != "$ligne" ]
                then
                    basename $pointeur >> $namedir
                fi
            done
            #cat $namedir 
            md5sum $namedir | cut -d' ' -f1 >> $temp2
            > $namedir
        done < $fichier
        cat $temp2 > $fichier
        rm $temp2
    ...
    echo "fichierTri" ; cat $fichierTri
    ...
    then
        ...
        then
            ...
            if [ "$type" = "f" ] #sinon c'est le rep 2 et les variables auront un nom ...2
            then
...
cheminCompletF(){
    ...
    then
        ...
        then
            printf "\nLes deux arborescences \e[34m\e[1m$dir1 \e[0met \e[34m\e[1m$dir2 \e[0ssont \e[1m\e[32mIDENTIQUES\e[0m\n" #on considère que les arbo sont pareils
        else
            printf "\nLes deux arborescences \e[34m\e[1m$dir1 \e[0met \e[34m\e[1m$dir2 \e[0ssont \e[1m\e[31mDIFFERENTES\e[0m\n"
        fi
        rm $fichierBrut
